...
/**
 * Description: <p>solr搜索结果 分页支持类</p>
 * 
 */
public class PageSupport<T> {
	
	private static final Logger logger = Logger.getLogger(PageSupport.class);
	...
	//分类
	private List<Category> facets = new LinkedList<Category>();
	...
	public PageSupport(List<T> items, int totalCount) {
	...
	public PageSupport(List<T> items, int totalCount, int offset) {
		setMaxPageItems(DEFAULT_MAX_PAGE_ITEMS);
		setTotalCount(totalCount);
		setItems(items);
		setOffset(offset);
	...
	public List<Category> getFacets() {
		return facets;
	}

	public void setFacets(List<Category> facets) {
		this.facets = facets;
	}
	...
	public static void main(String[] args) throws Exception {
		PageSupport<String> ps = new PageSupport<String>(null, 30, 10, 10);
